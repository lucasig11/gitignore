...
async function run() {
    ...
    log(formatLog(dedup, (file) => `<green><b>Adding:</b></green> <magenta>${file}</magenta> to .gitignore`));
    log(formatLog(skipLog, (file) =>`<yellow>Skipping:</yellow> <magenta>${file}</magenta> is already ignored`));

    await Deno.writeFile(".gitignore", encoder.encode(dedup.join("\n") + "\n"), { append: true });

    console.log(ink.colorize(`<green><b>Done!</b></green> Added <green>${dedup.length}</green> new entries. Skipped <yellow>${skipLog.length}</yellow>.`));
}
...
function formatLog(log: string[], format: (file: string) => string) {
    return log.map(file => ink.colorize(`    ${format(file)}`)).join("\n");
}
